"""Pairwise surface-to-surface gaps between the instances of a cell.

Moved from ``analyze_cell.py`` (algorithm unchanged), with the volumes passed in by
entity name so it can be driven from a PixelPatrol record's channels.
"""
...
def pairwise_instance_gaps(
    ...
    for a_id in range(1, gid + 1):
        ...
        dt_a = edt.edt(np.ascontiguousarray((sub != a_id).astype(np.uint8)),
                       anisotropy=(vz, vy, vx), parallel=1)
        o_ids = sub[other]
        o_dist = dt_a[other]
