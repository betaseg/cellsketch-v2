...
from pixel_patrol_anatomy import skeletons
from pixel_patrol_anatomy.mesh import MeshOptions, mesh_rows_for_object
...
from pixel_patrol_anatomy.skeletons import CACHE, parse_entity_filter, wants_skeletons
...
def test_excluded_entities_report_skeleton_metrics_as_not_measured(monkeypatch):
    monkeypatch.setenv("PP_ANATOMY_SKELETON_ENTITIES", "mito")
...
def test_skeleton_metrics_are_measured_for_everything_by_default(monkeypatch):
    monkeypatch.delenv("PP_ANATOMY_SKELETON_ENTITIES", raising=False)
    record = _object(mito=(_strand(), "label"), granules=(_strand(1, 12), "label"))
...
def test_a_different_object_is_not_served_from_the_cache(monkeypatch):
    ...
    InstanceProcessor().run_chunk(_object(mito=(_strand(), "label")))
    InstanceProcessor().run_chunk(_object(mito=(_strand(1, 12), "label")))
...
def test_geometry_matches_whether_it_was_cached_or_not():
    ...
    CACHE.clear()
    InstanceProcessor().run_chunk(_object(mito=(volumes["mito"], "label")))
...
def test_contacts_are_found_once_however_many_readers_ask(monkeypatch):
    import pixel_patrol_anatomy.contacts as contacts_module
    ...
    mesh_rows_for_object(volumes, kinds, VOXEL, object_id="object_a",
                       object_mask_name=record.meta["object_mask_name"],
