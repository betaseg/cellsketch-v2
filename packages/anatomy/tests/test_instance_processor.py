...
def _object(**entities: tuple[np.ndarray, str]):
    ...
    meta = {
        ...
        "object_id": "object_a",
        "object_mask_name": "pm" if "pm" in names else None,
        "object_shape": list(SHAPE),
...
def test_one_element_per_instance_across_all_entities():
    ...
    row = InstanceProcessor().run_chunk(_object(mito=(mito, "label"), er=(er, "label")))
...
def test_masks_contribute_no_instances_but_are_distance_targets():
    ...
    row = InstanceProcessor().run_chunk(_object(mito=(mito, "label"), nucleus=(nucleus, "mask")))
...
def test_distance_to_the_object_mask_is_distance_to_the_boundary():
    ...
    row = InstanceProcessor().run_chunk(_object(pm=(pm, "mask"), mito=(mito, "label")))
...
def test_closest_same_type_is_null_for_a_lone_instance():
    row = InstanceProcessor().run_chunk(_object(mito=(_blocks((1, (2, 2, 2), (3, 3, 3))), "label")))
...
def test_object_volume_comes_from_the_object_mask():
    pm = np.zeros(SHAPE, dtype=np.int32)
    pm[1:9, 1:19, 1:19] = 1
    row = InstanceProcessor().run_chunk(_object(pm=(pm, "mask")))

    assert row["object_volume_um3"] == pytest.approx(8 * 18 * 18 * np.prod(VOXEL))


def test_an_object_with_no_labelled_instances_emits_null_lists():
    pm = np.ones(SHAPE, dtype=np.int32)
    row = InstanceProcessor().run_chunk(_object(pm=(pm, "mask")))

    # None, not []: an empty list would type the parquet column List(Null) and clash
    # with the typed lists written for objects that do have instances.
...
def test_polarity_spread_grows_with_the_directions_an_instance_covers(monkeypatch):
    monkeypatch.setenv("PP_ANATOMY_POLARITY_SPREAD", "1")
    pm = np.zeros(SHAPE, dtype=np.int32)
    pm[1:9, 1:19, 1:19] = 1
...
def test_distance_histograms_share_their_bins_across_an_entity(monkeypatch):
    ...
    monkeypatch.setenv("PP_ANATOMY_DISTANCE_HISTOGRAMS", "1")
...
def test_partial_object_is_refused():
    record = _object(mito=(_blocks((1, (2, 2, 2), (3, 3, 3))), "label"))
    record.meta["object_shape"] = [10, 40, 40]
