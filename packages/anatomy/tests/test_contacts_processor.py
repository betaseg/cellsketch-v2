...
def _object(**entities: tuple[np.ndarray, str]):
    """An object record whose channels are the given name → (volume, kind) entities."""
    names = list(entities)
...
def test_touching_instances_report_the_smallest_possible_gap():
    ...
    row = ContactsProcessor().run_chunk(_object(mito=(mito, "label")))
...
def test_pairs_beyond_the_threshold_are_not_recorded(monkeypatch):
    monkeypatch.setenv("PP_ANATOMY_CONTACT_MAX_UM", "0.05")
    ...
    # with the lists written for objects that do have contacts.
...
def test_contacts_span_entities_and_mark_masks_with_a_null_label():
    ...
    row = ContactsProcessor().run_chunk(
        _object(mito=(mito, "label"), nucleus=(nucleus, "mask"))
    )
...
def test_the_object_mask_is_not_a_contact_partner():
    ...
    row = ContactsProcessor().run_chunk(_object(pm=(pm, "mask"), mito=(mito, "label")))
...
def test_an_object_with_one_instance_has_no_pairs():
    row = ContactsProcessor().run_chunk(_object(mito=(_blocks((1, (2, 2, 2), (3, 3, 3))), "label")))
...
def test_partial_object_is_refused():
    mito = _blocks((1, (2, 2, 2), (3, 3, 3)))
    record = _object(mito=(mito, "label"))
    record.meta["object_shape"] = [10, 40, 40]   # as if the volume had been split
